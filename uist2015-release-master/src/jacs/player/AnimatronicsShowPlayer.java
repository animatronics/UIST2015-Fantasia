...
public class AnimatronicsShowPlayer {
	...
	// Synchornized threads controlled by a CyclicBarrier
	private CyclicBarrier barrier;
	private final int DEFAULT_NUMBER_THREADS = 2; // 3;
	private int numBarrierThreads = DEFAULT_NUMBER_THREADS;
	...
	public AnimatronicsShowPlayer(MicrocontrollerConnection mc) throws Exception {
		microConnection = mc;
		// Default timer settings - 30 frames per second
		timingSettings = new TimingSettings(30, 30);
	}

	public void setMicrocontrollerConnection(MicrocontrollerConnection mc) {
		microConnection = mc;
	}

	public void setTimingSettings(TimingSettings t) {
		timingSettings = t;
	}

	public MicrocontrollerConnection getMicrocontrollerConnection() {
		return microConnection;
	}

	public TimingSettings getTimerSettings() {
	...
	public void playShow(FormattedShowData data, long startTime, long endTime) throws Exception {
		...
		// Interleave separate tracks to one
	...
	private void startSynchronizedShowTasks(String audioFile, int numServos, long startTime, long endTime,
		...
		numBarrierThreads = DEFAULT_NUMBER_THREADS; // Reset for each show
	...
	private class ServoPlayer implements Runnable {
		...
		public void run() {
			try {
				...
				// System.out.println("interrupted");	//中断した
			} catch (BrokenBarrierException e) {
